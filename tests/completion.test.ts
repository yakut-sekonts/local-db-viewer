...
test('columns resolve aliases defined after cursor, distinguish ambiguous names and ignore unrelated tables', () => {
  ...
  assert.equal(aggregate[0].insertText, '"id"');
});
test('quoted aliases, partial identifier replacement and dialect quotes', () => {
  ...
  assert.equal(suggest('SELECT o.| FROM orders o', 'mysql')[0].insertText, '`id`');
  assert.equal(suggest('SELECT o.| FROM orders o', 'mssql')[0].insertText, '[id]');
...
test('JOIN completion uses every FK pair and aliases, forward and reverse', () => {
  ...
  assert.equal(joins[0].insertText, '"public"."customers" "c" ON "o"."customer_id" = "c"."id" AND "o"."tenant_id" = "c"."tenant_id"');
  ...
  assert.equal(on[0].insertText, '"o"."customer_id" = "x"."id" AND "o"."tenant_id" = "x"."tenant_id"');
});
...
test('schema loader preserves composite key order and discards truncated foreign keys', async () => {
  ...
  assert.deepEqual((await run(false)).relationships[0].columns, relationship.columns);
...
test('virtual relationship persistence is atomic and isolates profiles; invalid pairs fail', async () => {
  ...
  assert.throws(() => validateRelation({ ...virtual, columns: [virtual.columns[0], virtual.columns[0]] }));
  await Promise.all([store.change('a', () => [virtual]), store.change('b', () => [{ ...virtual, id: 'b' }])]);
  assert.equal((await new RelationStore(path).list('a'))[0].id, relationship.id);
  assert.equal((await store.list('b'))[0].id, 'b');
