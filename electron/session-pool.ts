...
export interface SessionLease { profileId: string; templateId: string; session: DatabaseSession; release(): Promise<void> }
...
export class SessionPool {
  ...
  async acquire(connection: Connection, owner: string = crypto.randomUUID()): Promise<SessionLease> {
    const key = connection.jdbc?.options?.singleSession ? JSON.stringify(['profile', connection.id, connection.sessionTemplateId ?? '']) : `console:${owner}`;
    ...
    try {
      ...
      return { profileId: connection.id, templateId: connection.sessionTemplateId ?? '', session, release: async () => { if (!released) { released = true; await release(); } } };
