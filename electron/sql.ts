...
export function singleStatement(sql: string, engine: DatabaseEngine = 'jdbc', options: SqlLexingOptions = {}): string {
  ...
  let i = 0, end = sql.length, ended = false, hasToken = false;
  while (i < sql.length) {
    ...
    if (character === ';') { ended = true; end = i++; continue; }
    hasToken = true;
  ...
  if (!hasToken || !statement) throw new Error('SQL пустой или превышает 1 MB.');
  return statement;
}
