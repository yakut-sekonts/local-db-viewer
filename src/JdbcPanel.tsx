...
export function JdbcOptions({ draft, onChange, onError }: { draft: ProfileDraft; onChange(draft: ProfileDraft): void; onError(message: string): void }) {
  ...
  return <div className="jdbc-options">
    <h3>Connection</h3>
    <label>Switch schema<select aria-label="Switch schema" value={options.switchSchema ?? 'automatic'} onChange={event => change({ switchSchema: event.target.value as 'automatic' | 'manual' | 'disabled' })}><option value="automatic">Automatic</option><option value="manual">Manual</option><option value="disabled">Disable</option></select><small>Automatic восстанавливает контекст консоли и PostgreSQL search_path. Manual применяет только ваш выбор catalog/schema перед следующим запросом; после перезапуска используется контекст подключения. Disable запрещает IDE переключать контекст; SQL-команды USE / SET доступны во всех режимах.</small></label>
    ...
    <h3>Introspection</h3>
    <label>Load sources for<select aria-label="Load sources for" value={options.loadSources ?? 'user'} onChange={event => change({ loadSources: event.target.value as 'all' | 'user' | 'none' })}><option value="all">All schemas</option><option value="user">All excl. system schemas</option><option value="none">None</option></select><small>Исходники views, routines и triggers загружаются для индексируемых схем. Просмотр доступен через «Исходники объектов»; поддержка зависит от СУБД и прав подключения.</small></label>
    <label className="checkbox-row"><input type="checkbox" checked={options.preIntrospectedObjects ?? true} onChange={event => change({ preIntrospectedObjects: event.target.checked })} />Use pre-introspected objects for system catalogs</label>
    <small>Встроенный справочник для подсказок по системным каталогам, которые не индексируются. Реальные метаданные имеют приоритет; справочник не добавляет объекты в Explorer.</small>
    ...
    <h3>Code style</h3>
    <label>SQL keyword case<select aria-label="SQL keyword case" value={options.codeStyle?.keywordCase ?? 'upper'} onChange={event => change({ codeStyle: { indentSize: 4, useTabs: false, ...options.codeStyle, keywordCase: event.target.value as 'upper' | 'lower' | 'preserve' } })}><option value="upper">UPPER CASE</option><option value="lower">lower case</option><option value="preserve">Preserve</option></select></label>
    <label>Indent size<input aria-label="Indent size" type="number" min={1} max={8} value={options.codeStyle?.indentSize ?? 4} onChange={event => change({ codeStyle: { keywordCase: 'upper', useTabs: false, ...options.codeStyle, indentSize: Number(event.target.value) } })} /></label>
    <label className="checkbox-row"><input type="checkbox" checked={options.codeStyle?.useTabs ?? false} onChange={event => change({ codeStyle: { keywordCase: 'upper', indentSize: 4, ...options.codeStyle, useTabs: event.target.checked } })} />Use tabs</label>
    <small>Стиль применяется к новым подсказкам и SELECT preview. Существующий SQL форматируется по кнопке или Ctrl / ⌘ + Alt + L; выделение форматируется отдельно. Отмена — Ctrl / ⌘ + Z.</small>
