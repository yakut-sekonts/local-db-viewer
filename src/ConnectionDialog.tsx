    ...
    <form onSubmit={event => { event.preventDefault(); void perform('save'); }}>
      ...
      <div className="dialog-body">
        ...
        <label>СУБД<select aria-label="СУБД" value={draft.jdbc?.productId || draft.engine} disabled={!!profile || !!busy} onChange={event => {
          const product = DATABASE_PRODUCTS.find(product => product.id === event.target.value)!;
          const engine = product.engine, driver = driverDefinition(product.driverId);
          setDraft(current => ({ ...current, engine, endpoint: engine === 'jdbc' ? driver.url : ENGINES[engine].endpoint, user: ENGINES[engine].user, auth: ['trino', 'clickhouse', 'sqlite'].includes(engine) ? 'none' : 'basic', tls: !['trino', 'clickhouse', 'sqlite', 'jdbc'].includes(engine), sslVerification: 'FULL', sslCa: '', secret: undefined, catalog: '', schema: '', jdbc: { driverId: product.driverId, productId: product.id }, jdbcSecrets: [], jdbcEnvironmentNames: [] })); setMessage('');
        }}>{DATABASE_PRODUCTS.map(product => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label>
        ...
        <label>{draft.engine === 'sqlite' ? 'SQLite-файл' : draft.engine === 'trino' ? 'Coordinator URL' : 'URL подключения'}<div className="endpoint-field"><input required type={['sqlite', 'jdbc'].includes(draft.engine) ? 'text' : 'url'} placeholder={ENGINES[draft.engine].endpoint || '/path/to/database.sqlite'} value={draft.endpoint} onChange={event => update('endpoint', event.target.value)} />{draft.engine === 'sqlite' && <button type="button" className="button secondary" onClick={() => void window.studio.files.database().then(path => { if (path) update('endpoint', path); }).catch(error => setMessage(error.message))}>Выбрать</button>}</div><small>{draft.engine === 'trino' ? 'Адрес coordinator или reverse proxy, без /v1/statement.' : draft.engine === 'sqlite' ? 'Локальный файл SQLite. Изменения записываются непосредственно в файл.' : draft.engine === 'jdbc' ? 'JDBC URL производителя. Credentials и секретные параметры задайте в отдельных полях или Advanced.' : draft.engine === 'clickhouse' ? 'HTTP endpoint ClickHouse. Имя базы укажите отдельно в поле Database.' : 'Имя базы можно указать в URL после порта. Credentials — в отдельных полях.'}</small></label>
        {draft.engine !== 'sqlite' && <div className="form-row"><label>Пользователь<input required={draft.engine !== 'jdbc'} placeholder="username" value={draft.user} onChange={event => update('user', event.target.value)} /></label><label>Аутентификация<select value={draft.auth} onChange={event => update('auth', event.target.value)}><option value="none">Без пароля</option><option value="basic">Username / password</option>{draft.engine === 'trino' && <option value="bearer">Bearer token</option>}</select></label></div>}
        ...
        {draft.engine !== 'sqlite' && <div className="form-row">{draft.engine !== 'postgres' && draft.engine !== 'mssql' && <label>{draft.engine === 'trino' ? 'Catalog' : 'Database'} <span>необязательно</span><input placeholder="по умолчанию" value={draft.catalog} onChange={event => update('catalog', event.target.value)} /></label>}{['trino', 'postgres', 'jdbc'].includes(draft.engine) && <label>Schema <span>необязательно</span><input placeholder="по умолчанию" value={draft.schema} onChange={event => update('schema', event.target.value)} /></label>}</div>}
        {!['sqlite', 'jdbc'].includes(draft.engine) && <button type="button" className="ssl-summary" onClick={() => setSection('ssl')}>SSL = {String(sslEnabled(draft))} · SSLVerification = {draft.sslVerification ?? 'FULL'} · Настроить…</button>}
        <DriverSelection draft={draft} onChange={setDraft} />
        </div>
