...
/** Offline CREATE TABLE + FK model. Unsupported statements never execute. */
export function ddlIndex(mapping: DdlMapping, files: { file: string; sql: string }[], engine: DatabaseEngine): SchemaIndex {
  ...
  filesLoop: for (const file of files) {
    ...
    const tokens = all.filter(token => token.kind !== 'comment'); let created = 0;
    ...
    if (!created && file.sql.trim()) warn(file.file,'для подсказок поддерживается CREATE TABLE с явными колонками и REFERENCES; остальные команды сохранены как SQL');
    if (tokens.some(token => token.container === 0 && word(token,'ALTER'))) warn(file.file,'ALTER не изменяет локальный индекс; обновите CREATE TABLE до итоговой структуры');
  }
