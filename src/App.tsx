...
import { Database, History, Settings2, Plus, Play, Square, X, Terminal, ChevronDown, FolderOpen, Save, Command, Activity, Circle, PanelLeftClose, Folder, Files, Menu, ChevronRight, Link2, RefreshCw } from 'lucide-react';
...
interface Tab { id: string; name: string; sql: string; profileId: string; catalog: string; schema: string; result?: QuerySnapshot }
...
function restoreTabs(): Tab[] {
  try {
    ...
    if (Array.isArray(items)) {
      ...
      if (valid.length) return valid.map((item: Tab) => ({ id: crypto.randomUUID(), name: item.name, sql: item.sql, profileId: item.profileId, catalog: item.catalog, schema: item.schema }));
    }
...
function restoreHistory(): HistoryItem[] {
  try {
    ...
    return Array.isArray(data) ? data.filter(item => typeof item?.sql === 'string' && typeof item?.time === 'number').slice(0, 100) : [];
  } catch { return []; }
}
...
export function App() {
  ...
  const completion = useSchema(profile, tab.catalog, tab.schema, schemaRevision);
  ...
  function addTab(sql = INITIAL_SQL, name?: string, selectedProfile = profile, context?: { catalog: string; schema: string }) {
  ...
  async function closeTab(item: Tab) {
    try {
      ...
      setTabs(remaining);
      if (activeId === item.id) setActiveId(remaining[remaining.length - 1].id);
    } catch (error) { setError((error as Error).message); }
  }
  async function selectProfile(id: string) {
    try {
      await window.studio.query.release(tab.id);
      const next = profiles.find(item => item.id === id);
      updateTab({ profileId: id, catalog: next?.catalog ?? '', schema: next?.schema ?? '', result: undefined });
    } catch (error) { setError((error as Error).message); }
  }
  async function run(sql: string) {
    ...
    try { await window.studio.query.run({ requestId, sessionId: tab.id, profileId: profile.id, sql, catalog: tab.catalog, schema: tab.schema, maxRows }); }
    catch (error) {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      if (!(event.metaKey || event.ctrlKey) || dialog || relationsDialog) return;
  ...
  return <div className="app-shell">
    ...
    <div className="app-body" style={{ gridTemplateColumns: `${sidebar ? sidebarWidth : 0}px ${sidebar ? 3 : 0}px minmax(360px, 1fr) ${filesPanel ? 3 : 0}px ${filesPanel ? filesWidth : 0}px`, gridTemplateRows: `minmax(230px, 1fr) 4px ${servicesHeight}px` }}>
      {sidebar && <aside className="sidebar">{panel === 'database' ? <Explorer profile={profile} onAdd={() => setDialog({})} onSelectSchema={(catalog, schema) => { if (!running && !tab.result?.inTransaction) updateTab({ catalog, schema }); }} onRefresh={() => setSchemaRevision(value => value + 1)} onPreview={(catalog, schema, table) => {
        addTab(previewSQL(profile!.engine, catalog, schema, table), `${table}.sql`, profile, { catalog, schema });
      }} /> : <><div className="panel-heading"><span>ИСТОРИЯ ЗАПРОСОВ</span><span className="count">{history.length}</span></div><div className="history-list">{!history.length && <div className="explorer-empty"><History size={27} /><h3>Каждый запрос под рукой</h3><p>Здесь будут последние 100<br />выполненных запросов.</p></div>}{history.map(item => <button key={item.id} className="history-item" onClick={() => addTab(item.sql, 'history.sql', profiles.find(profile => profile.id === item.profileId), { catalog: item.catalog, schema: item.schema })}><div><span className={`history-dot ${item.state.toLowerCase()}`} /><strong>{item.profileName}</strong><time>{new Date(item.time).toLocaleTimeString('ru', { hour: '2-digit', minute: '2-digit' })}</time></div><code>{item.sql}</code><small>{labelState(item.state)} · {(item.duration / 1000).toFixed(2)} s</small></button>)}</div><div className="sidebar-foot">Нажмите запрос, чтобы открыть в консоли</div></>}</aside>}
      ...
      <main className="workspace">
        ...
        <div className="query-toolbar"><div className="connection-select"><Database size={14} /><select aria-label="Подключение" value={profile?.id ?? ''} disabled={running || tab.result?.inTransaction} onChange={event => void selectProfile(event.target.value)}><option value="">Выберите подключение</option>{profiles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><ChevronDown size={12} /></div><div className="toolbar-separator" /><label className="context-input"><span>{profile?.engine === 'trino' ? 'catalog' : 'database'}</span><input aria-label="Catalog" placeholder="по умолчанию" value={tab.catalog} disabled={running || (profile && ['postgres', 'sqlite', 'mssql'].includes(profile.engine))} onChange={event => updateTab({ catalog: event.target.value })} /></label><span className="context-slash">/</span><label className="context-input"><span>schema</span><input aria-label="Schema" placeholder="по умолчанию" value={tab.schema} disabled={running || tab.result?.inTransaction || (profile && !['trino', 'postgres', 'mssql'].includes(profile.engine))} onChange={event => updateTab({ schema: event.target.value })} /></label><div className="spacer" />{running ? <button className="button cancel-button" onClick={() => void window.studio.query.cancel(tab.result!.requestId).catch(error => setError(error.message))}><Square size={12} fill="currentColor" />Отменить</button> : <button className="button run-button" onClick={() => void run(editor.current?.selection() ?? tab.sql)}><Play size={13} fill="currentColor" />Выполнить<kbd>Ctrl / ⌘ ↵</kbd></button>}</div>
        ...
        <div className="editor-footer"><span><Circle size={7} fill="currentColor" />{profile ? ENGINES[profile.engine].name : 'SQL'}</span><span>UTF-8</span><button className="completion-status" title={completion.status || "Автодополнение колонок и JOIN: Ctrl + Space"} onClick={() => { if (completion.status) setError(completion.status); }}>{completion.busy ? "Индексирование…" : completion.status ? "Метаданные: есть замечания" : completion.index ? `${completion.index.tables.length} tables · ${completion.index.relationships.length} links` : "Ctrl + Space"}</button><button className="icon-button" aria-label="Обновить автодополнение" disabled={!profile || completion.busy} onClick={() => setSchemaRevision(value => value + 1)}><RefreshCw size={12} /></button><button className="icon-button" aria-label="Связи таблиц" title="Foreign keys и виртуальные связи" disabled={!completion.index} onClick={() => setRelationsDialog(completion.index)}><Link2 size={14} /></button><div className="spacer" /><label>Сохранять строк<select aria-label="Лимит строк" value={maxRows} disabled={running} onChange={event => setMaxRows(Number(event.target.value))}><option value={100}>100</option><option value={1000}>1 000</option><option value={5000}>5 000</option><option value={10000}>10 000</option></select></label></div>
      </main>
