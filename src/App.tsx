...
interface Tab { searchPath?: string; applyContext?: boolean; ddlMappingId?: string; templateId?: string; id: string; name: string; sql: string; profileId: string; catalog: string; schema: string; result?: QuerySnapshot }
...
export function App() {
  ...
  useEffect(() => window.studio.query.onUpdate(result => {
    ...
    if (!request) return;
    setTabs(items => items.map(item => item.id === request.tabId ? { ...item, result, catalog: result.catalog ?? item.catalog, schema: result.schema ?? item.schema, searchPath: result.searchPath ?? item.searchPath, applyContext: result.contextApplied ? false : item.applyContext } : item));
  ...
  async function selectProfile(id: string) {
    try {
      ...
      updateTab({ ddlMappingId: undefined, templateId: undefined, profileId: id, catalog: next?.catalog ?? '', schema: next?.schema ?? '', searchPath: undefined, applyContext: false, result: undefined });
    } catch (error) { setError((error as Error).message); }
  }
  async function run(sql: string) {
    ...
    try { await window.studio.query.run({ requestId, sessionId: tab.id, profileId: profile.id, templateId: tab.templateId, ddlMappingId: tab.ddlMappingId, sql, catalog: tab.catalog, schema: tab.schema, searchPath: tab.searchPath, applyContext: tab.applyContext, maxRows }); }
    catch (error) {
  ...
  return <div className="app-shell">
    ...
    <div className="app-body" style={{ gridTemplateColumns: `${sidebar ? sidebarWidth : 0}px ${sidebar ? 3 : 0}px minmax(360px, 1fr) ${filesPanel ? 3 : 0}px ${filesPanel ? filesWidth : 0}px`, gridTemplateRows: `minmax(230px, 1fr) 4px ${servicesHeight}px` }}>
      ...
      <main className="workspace">
        ...
        <div className="editor-pane"><SqlEditor key={tab.id} ref={editor} value={tab.sql} onChange={sql => updateTab({ sql })} onRun={sql => void run(sql)} engine={profile?.engine ?? 'trino'} driverId={profile ? profileDriver(profile) : undefined} codeStyle={profile?.jdbc?.options?.codeStyle} onError={setError} getSchema={completion.forQuery} /></div>
