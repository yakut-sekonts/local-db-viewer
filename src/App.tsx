...
import { ENGINES, executionState, type Profile, type QuerySnapshot, type SchemaIndex } from './shared';
...
export function App() {
  ...
  async function run(sql: string, mode: 'statement' | 'script' = 'statement') {
    ...
    requests.current.set(requestId, { mode, tabId: tab.id, sql, profileId: profile.id, catalog: tab.catalog, schema: tab.schema });
    updateTab({ result, scriptResults: undefined }); setError('');
    try { await window.studio.query.run({ mode, requestId, sessionId: tab.id, profileId: profile.id, templateId: tab.templateId, ddlMappingId: tab.ddlMappingId, sql, catalog: tab.catalog, schema: tab.schema, searchPath: tab.searchPath, applyContext: tab.applyContext, maxRows }); }
    catch (error) {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      if (!(event.metaKey || event.ctrlKey) || dialog || ddlDialog || sourcesDialog || relationsDialog) return;
  ...
  return <div className="app-shell">
    ...
    <div className="app-body" style={{ gridTemplateColumns: `${sidebar ? sidebarWidth : 0}px ${sidebar ? 3 : 0}px minmax(360px, 1fr) ${filesPanel ? 3 : 0}px ${filesPanel ? filesWidth : 0}px`, gridTemplateRows: `minmax(230px, 1fr) 4px ${servicesHeight}px` }}>
      ...
      <main className="workspace">
        ...
        <div className="editor-pane"><SqlEditor key={tab.id} ref={editor} value={tab.sql} onChange={sql => updateTab({ sql })} onRun={sql => void run(sql)} onScript={sql => void run(sql, 'script')} engine={profile?.engine ?? 'trino'} driverId={profile ? profileDriver(profile) : undefined} codeStyle={profile?.jdbc?.options?.codeStyle} onError={setError} getSchema={completion.forQuery} /></div>
