...
export function SSLPanel({ draft, onChange, onError, busy }: { draft: ProfileDraft; onChange(draft: ProfileDraft): void; onError(message: string): void; busy: boolean }) {
  ...
  if (draft.engine === 'jdbc') return <p className="settings-note">TLS настраивается параметрами выбранного JDBC-драйвера в Advanced или JDBC URL. Используйте документацию производителя: названия SSL-параметров различаются между драйверами.</p>;
  ...
  return <div className="ssl-panel">
    ...
    <label>CA certificate / bundle<div className="certificate-actions"><button className="button secondary" type="button" disabled={!enabled || busy} onClick={() => void importCA()}><Upload size={14} />Импорт PEM</button><span>{draft.sslCa ? 'Сертификат сохранён в подключении' : draft.jdbc ? 'Truststore JDBC-драйвера' : 'Системные и встроенные CA'}</span>{draft.sslCa && <button className="icon-button" type="button" aria-label="Удалить CA" disabled={busy} onClick={() => onChange({ ...draft, sslCa: '' })}><Trash2 size={14} /></button>}</div><textarea aria-label="CA certificate PEM" rows={6} placeholder="-----BEGIN CERTIFICATE-----" value={draft.sslCa ?? ''} disabled={!enabled || busy} onChange={event => onChange({ ...draft, sslCa: event.target.value })} /><small>Публичные сертификаты PEM до 256 KB. {draft.jdbc ? 'В JDBC это явный доверенный CA bundle. Системный truststore и клиентские сертификаты настраиваются параметрами драйвера в Advanced.' : 'Заданный CA bundle заменяет системные и встроенные корни доверия.'}</small></label>
